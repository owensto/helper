...
try{
 for(const [name,width,height] of [['desktop',1600,1000],['phone',430,932]]){
  ...
  await page.goto(process.env.PUBLIC_BASE||'http://127.0.0.1:8765/',{waitUntil:'networkidle',timeout:90000});
  await page.waitForFunction(()=>window.ATLAS_D?.ready,{},{timeout:90000});await page.waitForFunction(()=>window.ATLAS_D?.report,{},{timeout:30000});await page.waitForTimeout(1800);
  ...
  await page.screenshot({timeout:120000,path:path.join(out,name+'_surface.png')});
  await page.locator('[data-view="profile"]').click();await page.waitForTimeout(700);await page.screenshot({timeout:120000,path:path.join(out,name+'_profile.png')});
  ...
  assert.ok(await page.evaluate(()=>ATLAS_D.visibleMeshes>5));await page.screenshot({timeout:120000,path:path.join(out,name+'_submerged.png')});
  await page.locator('[data-mode="cutaway"]').click();await page.waitForTimeout(900);assert.equal(await page.evaluate(()=>ATLAS_D.state),'cutaway');await page.screenshot({timeout:120000,path:path.join(out,name+'_cutaway.png')});
  await page.locator('[data-system="I_Energy"]').click();await page.waitForTimeout(500);
  await page.locator('[data-mode="surface"]').click();await page.locator('[data-view="stern"]').click();await page.waitForTimeout(1000);await page.screenshot({timeout:120000,path:path.join(out,name+'_stern.png')});
  await page.locator('[data-view="fleet"]').click();await page.waitForTimeout(700);await page.screenshot({timeout:120000,path:path.join(out,name+'_tenders.png')});
