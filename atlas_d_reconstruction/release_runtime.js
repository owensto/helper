...
try{
 ...
 let redraw=true;controls.addEventListener('change',()=>{redraw=true;});document.addEventListener('click',()=>{redraw=true;});
 ...
 function opacity(o,value){const m=o.material;const original=o.userData.original;const alpha=original.opacity*value;
  m.opacity=alpha;const transparent=original.transparent||alpha<.999;if(m.transparent!==transparent){m.transparent=transparent;m.needsUpdate=true;}m.depthWrite=alpha>.94&&original.depthWrite;
 }
 ...
 function applyMode(next){
  redraw=true;renderer.shadowMap.needsUpdate=true;transition=null;mode=next;system='all';
 ...
 function frame(now){requestAnimationFrame(frame);if(document.hidden)return;const dt=Math.min((now-last)/1000,.06);last=now;
  if(spin||transition){waterNormal.offset.x+=dt*.0013;waterNormal.offset.y+=dt*.0005;redraw=true;}
  if(transition){
  ...
  controls.update();if(redraw||spin||transition){renderer.render(scene,camera);redraw=false;}
 }
